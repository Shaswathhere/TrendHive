...
export function DashboardLayout() {
  const { preferences, isWorkspaceLoading } = useWorkspace()
  const location = useLocation()

  if (isWorkspaceLoading) {
    return <div className="min-h-screen bg-[#fcfcff] dark:bg-[#0c0a1e] flex items-center justify-center">
      <div className="w-10 h-10 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin" />
    </div>
  }

  if (!preferences?.isOnboarded && location.pathname !== "/onboarding") {
